...
> 最後更新:2026-05-31(代碼淨化與收尾完成:全專案 pyflakes 零警告、無死碼/未用 import,本輪無需變更)
...
- [ ] 全市場化:Streamlit 按「🌐 匯入全台股 ETF 清單」帶入全市場 ETF,再按「🔄 抓取成分股」
      與「🔄 抓取 ETF 圖鑑」全清單重抓(Basic0004/0005,確認型態/配息/費用/月份)並自動存檔
...
- [~] (C9)抓不到的個別 ETF 部分處理:
      - 已修:`fetch_moneydj` 加 etfid 大小寫變體重試(主動式後綴需小寫 a,如 00983a.tw);
        修正 00983A/00984A 名稱(00983A=主動中信ARK創新、00984A=主動安聯台灣高息,原標錯)
      - 待線上驗證:0058/0059/00948/00950(被動式),需線上用「🔬 診斷單檔」看 MoneyDJ
        該頁是否有成分股表/正確 etfid;沙箱連不到 MoneyDJ 無法實測
