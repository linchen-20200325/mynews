...
3. 護城河與長期持有觀察(此題可動用你的【產業結構常識】補充,不限於新聞):
   ...
   - supply_chain:【務必填寫,不分台股/美股】列出與本檔同屬一條產業鏈、高度相關的上中下游代表個股;
     物件,含 upstream/midstream/downstream 三個陣列,元素為 {name, ticker, role 供應/角色};
     · 若本檔為美股或國際大廠:盡量列出對應的『台股』供應鏈(台股代號通常 4 碼數字);
     · 若本檔為台股:列出該產業鏈上中下游的代表台股(可含本檔所處的那一段)。
     ticker 不確定就留空,【嚴禁亂編代號】;真的找不到對應個股,該段才留空陣列。
...
def build_stock_query_user_prompt(
    ...
    return (
        ...
        f"並列出本檔所屬產業鏈的上中下游代表個股(美股→對應台股供應鏈;台股→該產業鏈台股,可含本檔),"
        f"最後給是否適合長期持有的中立研判"
