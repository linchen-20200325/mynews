...
【root_cause 鐵則 — 最重要，不可省略】
⚠️ root_cause 欄位是必填，若留空或省略，整個 JSON 視為無效。
- root_cause 必須是今日對市場影響最大的【單一事件】,一句話,20字內。
- 優先順序:地緣政治衝突/戰爭 > 重大政策衝擊(關稅/制裁/禁令) > 央行決策 > 重大財報利空 > 技術面修正。
- 例:「美伊開戰→油價飆→科技股遭拋售」「川普對華加關稅→貿易戰疑慮」「Fed 鷹派→升息預期升溫」「AI成本上升→科技股評估修正」
- 若新聞無法確認單一主因,必須寫:「市場自發修正,無單一明確主因」(不可留空)
